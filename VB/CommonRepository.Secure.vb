    ...
    Public Function ValidateUser(ByVal _validate As UserDTO) As Boolean
        ...
        If _validate.EMPLOYEE_CODE <> "" Then
            query = (From p In Context.HU_EMPLOYEE
                     Where p.EMPLOYEE_CODE.ToUpper = _validate.EMPLOYEE_CODE.ToUpper).FirstOrDefault
...
#Region "Group Function"
    ...
    Public Function GetGroupFunctionNotPermision(ByVal _groupID As Decimal,
                                                ...
                                                ByRef Total As Integer,
                                                Optional ByVal Sorts As String = "NAME asc") As List(Of FunctionDTO)
        Dim lstTemp = (From p In Context.SE_FUNCTION Select p)
        If _filter.MODULE_NAME <> "" Then
...
#Region "Check User Login"
    ...
    Public Function GetUserPermissions(ByVal Username As String) As List(Of PermissionDTO)
        Try
            ...
            Dim lstPermissions As List(Of PermissionDTO) = query1.Union(query2).ToList
    ...
    Public Function GetUserWithPermision(ByVal Username As String) As UserDTO
        Try
            ...
            Dim query = (From p In Context.SE_USER
                         ...
                         Order By p.EMPLOYEE_CODE Descending)
...
#Region "Student"
    ...
    Public Function GetStudentToPopupFind(ByVal _filter As StudentPopupFindListDTO,
                                            ByVal PageIndex As Integer,
                                            ByVal PageSize As Integer,
                                            ByRef Total As Integer,
                                            Optional ByVal Sorts As String = "Student_CODE asc",
                                            Optional ByVal log As UserLog = Nothing,
                                            Optional ByVal _param As ParamDTO = Nothing) As List(Of StudentPopupFindListDTO)
        ...
        Try
            ...
            If _filter.Student_CODE <> "" Then
                query = query.Where(Function(f) f.p.Student_CODE.ToUpper.Contains(_filter.Student_CODE.ToUpper) Or _
                                        f.p.FULLNAME_VN.ToUpper.Contains(_filter.Student_CODE.ToUpper))
            End If
            ...
            Dim lst = query.Select(Function(f) New StudentPopupFindListDTO With {
                            .ID = f.p.ID,
                            .STUDENT_ID = f.p.ID,
                            .STUDENT_CODE = f.p.STUDENT_CODE,
                            .FULLNAME_VN = f.p.FULLNAME_VN,
                            .JOIN_DATE = f.p.JOIN_DATE,
                            .ORG_NAME = f.o.NAME_VN,
                            .ORG_CODE = f.o.CODE,
    ...
    Public Function GetStudentToPopupFind_StudentID(ByVal _empId As List(Of Decimal)) As List(Of StudentPopupFindDTO)
...
#Region "Kitchen"
    ...
    Public Function GetKitchen(ByVal _filter As AT_KITCHEN_DTO, ByVal PageIndex As Integer,
                                    ByVal PageSize As Integer,
                                    ByRef Total As Integer,
        ...
        Try
            Dim query = From p In Context.AT_KITCHEN
                        Select New AT_KITCHEN_DTO With {
                                  .ID = p.ID,
            ...
            If _filter.KITCHEN_CODE <> "" Then
                lst = lst.Where(Function(p) p.KITCHEN_CODE.ToUpper.Contains(_filter.KITCHEN_CODE.ToUpper))
            End If
            If _filter.KITCHEN_NAME <> "" Then
                lst = lst.Where(Function(p) p.KITCHEN_NAME.ToUpper.Contains(_filter.KITCHEN_NAME.ToUpper))
            End If
            If _filter.REMARK <> "" Then
                lst = lst.Where(Function(p) p.REMARK.ToUpper.Contains(_filter.REMARK.ToUpper))
            End If
    ...
    Public Function GetKitchenFromId(ByVal _lstIds As List(Of Decimal)) As List(Of AT_KITCHEN_DTO)
        Try
            Dim query = (From p In Context.AT_KITCHEN Where _lstIds.Contains(p.ID)
                        Select New AT_KITCHEN_DTO With {
                            .ID = p.ID,
                            .KITCHEN_CODE = p.KITCHEN_CODE,
                            .KITCHEN_NAME = p.KITCHEN_NAME}).ToList
            Return query
        Catch ex As Exception
            Throw ex
        End Try
